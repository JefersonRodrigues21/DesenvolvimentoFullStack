...
//? Método MAP (Array.map()) - permite a você iterar sobre o array e modificar seus elementos usando uma função de callback. A função callback será executada em cada um dos elementos do array.
let arr = [3, 4, 5, 6];
...
nomes = funcionarios2.map(pessoa => pessoa.nome);
console.log(nomes);
...
let num = [1, 2, 3, 4, 5, 6];
let total = num.reduce(function(total, numero) {
    return total + numero;
}, 0); // Zero é o total.
console.log(total);
